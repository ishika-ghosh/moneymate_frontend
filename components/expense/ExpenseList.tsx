        ...
        expenseList.map((item: any, index: any) => (
          <Swipeable
            ...
            renderRightActions={() => <RightView item={item}
              ...
              handleUpdate={(_item: any) => updateItem(_item, index)}
            />}
            onSwipeableOpen={() => {
              closeRow(item, index);
            }}
  ...
  return (
    <View className="bg-gray-100 py-4 mb-2 pl-3 border-[0.3px] border-gray-40 rounded-lg flex flex-row items-center justify-between z-0">
      ...
      <View className="flex flex-row items-center justify-between basis-5/6 px-2 pr-5">
        <View>
          <Text className=" text-gray-10 text-[17px] font-semibold">{item.category.name}  </Text>
          <Text className="text-gray-30 text-sm font-JakartaLight mb-1">{item.desc}</Text>
          <Text className="text-gray-20 font-light">{moment(item.createdAt).format('MMMM Do YYYY, h:mm A')}</Text>
        </View>
